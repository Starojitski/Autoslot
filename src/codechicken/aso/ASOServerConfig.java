...
public class ASOServerConfig
{
    ...
    public static void load(World world) {
        if (MinecraftServer.getServer() != server) {
            logger.debug("Loading ASO Server");
            server = MinecraftServer.getServer();
            saveDir = new File(DimensionManager.getCurrentSaveRootDirectory(), "ASO");
    ...
    public static File getSaveDir(World world) {
        return new File(CommonUtils.getSaveLocation(world), "ASO");
    }

    private static void loadWorld(World world) {
        try {
            File file = new File(getSaveDir(world), "world.dat");
            NBTTagCompound tag = ASOServerUtils.readNBT(file);
    ...
    private static void saveWorld(int dim) {
        try {
            ...
            ASOServerUtils.writeNBT(dimTags.get(dim), file);
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    }
    ...
    public static boolean canPlayerPerformAction(String playername, String name) {
        return isPlayerInList(playername, getPlayerList("permissions." + ASOActions.base(name)), true);
    }
    ...
    public static void disableAction(int dim, String name, boolean disable) {
        ...
        ASOSPH.sendActionDisabled(dim, name, disable);
        saveWorld(dim);
    }
