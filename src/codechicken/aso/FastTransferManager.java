...
import static codechicken.aso.ASOServerUtils.*;
...
public class FastTransferManager
{
    ...
    public void performMassTransfer(GuiContainer window, int fromSlot, int toSlot, ItemStack heldStack) {
        ...
        if (ASOClientUtils.getHeldItem() != null && !areStacksSameType(heldStack, ASOClientUtils.getHeldItem()))
            return;
    ...
    public void moveOutputSet(GuiContainer window, int fromSlot, int toSlot) {
        ...
        if (ASOClientUtils.getHeldItem() == null)//maybe this container does auto transfers. No need to pick up the final item
            return;
    ...
    private boolean fillZoneWithHeldItem(GuiContainer window, int zoneIndex) {
        ...
        return ASOClientUtils.getHeldItem() == null;
    }
