...
import codechicken.aso.api.IConfigureASO;
import codechicken.aso.asm.ASOCorePlugin;
...
public class ASOModContainer extends DummyModContainer
{
    public static LinkedList<IConfigureASO> plugins = new LinkedList<IConfigureASO>();

    public ASOModContainer() {
        super(MetadataCollection.from(MetadataCollection.class.getResourceAsStream("/asomod.info"), "AutoSlot").getMetadataForId("AutoSlot", null));
        loadMetadata();
    }
    ...
    public ModMetadata getMetadata() {
        ...
        if (plugins.size() == 0) {
            ...
            for (int i = 0; i < plugins.size(); i++) {
                if (i > 0)
                    s_plugins += ", ";
                IConfigureASO plugin = plugins.get(i);
