...
import codechicken.aso.api.LayoutStyle;
import net.minecraft.client.gui.inventory.GuiContainer;
import net.minecraft.nbt.NBTTagCompound;

import static codechicken.lib.gui.GuiDraw.getStringWidth;
import static codechicken.aso.LayoutManager.*;
...
public abstract class LayoutStyleDefault extends LayoutStyle
{
    ...
    public void layout(GuiContainer gui, VisiblityData visiblity) {
        ...
        delete.state = 0x4;
        if (ASOController.getDeleteMode())
            delete.state |= 1;
        ...
        if (disabledActions.contains("rain"))
            rain.state |= 2;
        else if (ASOClientUtils.isRaining())
            rain.state |= 1;

        gamemode.state = 0x4;
        if (ASOClientUtils.getGamemode() != 0) {
            gamemode.state |= 0x1;
            gamemode.index = ASOClientUtils.getGamemode() - 1;
        } else {
            if (ASOClientUtils.isValidGamemode("creative"))
                gamemode.index = 0;
            else if (ASOClientUtils.isValidGamemode("creative+"))
                gamemode.index = 1;
            else if (ASOClientUtils.isValidGamemode("adventure"))
                gamemode.index = 2;
        }
        ...
        if (canPerformAction("rain"))
            layoutButton(rain);
        if (ASOClientUtils.isValidGamemode("creative") ||
                ASOClientUtils.isValidGamemode("creative+") ||
                ASOClientUtils.isValidGamemode("adventure"))
            layoutButton(gamemode);
        if (canPerformAction("magnet"))
            layoutButton(magnet);
        if (canPerformAction("time")) {
            for (int i = 0; i < 4; i++) {
                timeButtons[i].state = disabledActions.contains(ASOActions.timeZones[i]) ? 2 : 0;
