...
public class ContainerEnchantmentModifier extends ContainerEnchantment
{
    ...
    private void toggleSlotEnchantment(int slot) {
        ...
        if (e.state == 2) {
            ASOCPH.sendModifyEnchantment(e.enchantment.effectId, 0, false);
            e.state = 0;
        } else if (e.state == 1) {
            return;
        } else {
            ASOCPH.sendModifyEnchantment(e.enchantment.effectId, level, true);
            e.state = 2;
        }
    ...
    public void updateEnchantmentOptions(boolean validate) {
        ...
        for (Enchantment e : Enchantment.enchantmentsList) {
            ...
            int state = 0;
            int level = -1;
            if (ASOServerUtils.stackHasEnchantment(toolstack, e.effectId)) {
                state = 2;
                level = ASOServerUtils.getEnchantmentLevel(toolstack, e.effectId);
            } else if (ASOServerUtils.doesEnchantmentConflict(ASOServerUtils.getEnchantments(toolstack), e) && validate) {
                state = 1;
            }
