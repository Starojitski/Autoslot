...
import codechicken.aso.ItemList.PatternItemFilter;
...
import static codechicken.aso.ASOClientConfig.world;
...
public class SearchField extends TextField implements ItemFilterProvider
{
    ...
    public boolean handleClick(int mousex, int mousey, int button) {
        if (button == 0) {
            if (focused() && (System.currentTimeMillis() - lastclicktime < 500)) {//double click
                ASOClientConfig.world.nbt.setBoolean("searchinventories", !searchInventories());
    ...
    public void onTextChange(String oldText) {
        ASOClientConfig.setSearchExpression(text());
        ItemList.updateFilter.restart();
    }

    @Override
    public void lastKeyTyped(int keyID, char keyChar) {
        if (keyID == ASOClientConfig.getKeyBinding("gui.search"))
            setFocus(true);
    }
    ...
    public static Pattern getPattern(String search) {
        switch (ASOClientConfig.getIntSetting("inventory.searchmode")) {
            case 0://plain
                search = "\\Q" + search + "\\E";
                break;
            case 1:
