...
public class ASOSPH implements IServerPacketHandler
{
    @Override
    public void handlePacket(PacketCustom packet, EntityPlayerMP sender, INetHandlerPlayServer netHandler) {
        if (!ASOServerConfig.authenticatePacket(sender, packet))
            return;
        ...
        switch (packet.getType()) {
            case 1:
                ...
                break;
            case 4:
                ASOServerUtils.deleteAllItems(sender);
                break;
            case 5:
                setInventorySlot(sender, packet);
                break;
            case 6:
                ASOServerUtils.toggleMagnetMode(sender);
                break;
            case 7:
                ASOServerUtils.setHourForward(sender.worldObj, packet.readUByte(), true);
                break;
            case 8:
                ASOServerUtils.healPlayer(sender);
                break;
            case 9:
                ASOServerUtils.toggleRaining(sender.worldObj, true);
                break;
            case 10:
            ...
            case 12:
                ...
                break;
            case 13:
                ASOServerUtils.setGamemode(sender, packet.readUByte());
                break;
            case 14:
                ASOServerUtils.cycleCreativeInv(sender, packet.readInt());
                break;
            case 15:
    ...
    private void handlePropertyChange(EntityPlayerMP sender, PacketCustom packet) {
        ...
        if (ASOServerConfig.canPlayerPerformAction(sender.getCommandSenderName(), name))
            ASOServerConfig.disableAction(sender.dimension, name, packet.readBoolean());
    }

    public static void processCreativeInv(EntityPlayerMP sender, boolean open) {
        if (open) {
            ServerUtils.openSMPContainer(sender, new ContainerCreativeInv(sender, new ExtendedCreativeInv(ASOServerConfig.forPlayer(sender.getCommandSenderName()), Side.SERVER)), new IGuiPacketSender()
            {
                @Override
    ...
    private void handleGiveItem(EntityPlayerMP player, PacketCustom packet) {
        ASOServerUtils.givePlayerItem(player, packet.readItemStack(), packet.readBoolean(), packet.readBoolean());
    }
    ...
    private void setInventorySlot(EntityPlayerMP player, PacketCustom packet) {
        ...
        ItemStack old = ASOServerUtils.getSlotContents(player, slot, container);
        boolean deleting = item == null || old != null && ASOServerUtils.areStacksSameType(item, old) && item.stackSize < old.stackSize;
        if (ASOServerConfig.canPlayerPerformAction(player.getCommandSenderName(), deleting ? "delete" : "item"))
            ASOServerUtils.setSlotContents(player, slot, item, container);
    }
    ...
    private void sendLoginState(EntityPlayerMP player) {
        ...
        PlayerSave playerSave = ASOServerConfig.forPlayer(player.getCommandSenderName());

        for (String name : ASOActions.nameActionMap.keySet()) {
            if (ASOServerConfig.canPlayerPerformAction(player.getCommandSenderName(), name))
                actions.add(name);
            if (ASOServerConfig.isActionDisabled(player.dimension, name))
                disabled.add(name);
            if (playerSave.isActionEnabled(name))
                enabled.add(name);
        }
        for (ItemStackMap.Entry<Set<String>> entry : ASOServerConfig.bannedItems.entries())
            if (!ASOServerConfig.isPlayerInList(player.getCommandSenderName(), entry.value, true))
    ...
    public static void sendHasServerSideTo(EntityPlayerMP player) {
        ASOServerConfig.logger.debug("Sending serverside check to: " + player.getCommandSenderName());
        PacketCustom packet = new PacketCustom(channel, 1);
        packet.writeByte(ASOActions.protocol);
