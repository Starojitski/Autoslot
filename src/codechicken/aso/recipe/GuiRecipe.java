...
import codechicken.aso.*;
import codechicken.aso.LayoutManager;
import codechicken.aso.api.IGuiContainerOverlay;
import codechicken.aso.api.IOverlayHandler;
import codechicken.aso.api.IRecipeOverlayRenderer;
import codechicken.aso.guihook.GuiContainerManager;
import codechicken.aso.guihook.IContainerTooltipHandler;
import codechicken.aso.guihook.IGuiClientSide;
import codechicken.aso.guihook.IGuiHandleMouseWheel;
...
public abstract class GuiRecipe extends GuiContainer implements IGuiContainerOverlay, IGuiClientSide, IGuiHandleMouseWheel, IContainerTooltipHandler
{
    ...
    public void initGui() {
        ...
        GuiButton nexttype = new GuiASOButton(0, width / 2 - 70, (height - ySize) / 2 + 3, 13, 12, "<");
        GuiButton prevtype = new GuiASOButton(1, width / 2 + 57, (height - ySize) / 2 + 3, 13, 12, ">");
        nextpage = new GuiASOButton(2, width / 2 - 70, (height + ySize) / 2 - 18, 13, 12, "<");
        prevpage = new GuiASOButton(3, width / 2 + 57, (height + ySize) / 2 - 18, 13, 12, ">");
        overlay1 = new GuiASOButton(4, width / 2 + 65, (height - ySize) / 2 + 63, 13, 12, "?");
        overlay2 = new GuiASOButton(5, width / 2 + 65, (height - ySize) / 2 + 128, 13, 12, "?");
    ...
    public void keyTyped(char c, int i) {
        ...
        if (i == mc.gameSettings.keyBindInventory.getKeyCode())
            mc.displayGuiScreen(firstGui);
        else if (i == ASOClientConfig.getKeyBinding("gui.back"))
    ...
    private void overlayRecipe(int recipe) {
        ...
        boolean shift = ASOClientUtils.shiftKey();
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        ...
        s = ASOClientUtils.translate("recipe.page", page + 1, (currenthandlers.get(recipetype).numRecipes() - 1) / recipehandler.recipiesPerPage() + 1);
    ...
    protected void drawGuiContainerBackgroundLayer(float f, int mx, int my) {
        ...
        CCRenderState.changeTexture("aso:textures/gui/recipebg.png");
        int j = (width - xSize) / 2;
