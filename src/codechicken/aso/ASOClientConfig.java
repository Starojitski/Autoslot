...
import codechicken.aso.config.*;
import codechicken.aso.recipe.RecipeInfo;
...
public class ASOClientConfig
{
    ...
    private static void setDefaults() {
        ...
        API.addOption(new OptionCycled("inventory.cheatmode", 3)
        {
            ...
            public boolean optionValid(int index) {
                return getLockedMode() == -1 || getLockedMode() == index && ASOInfo.isValidMode(index);
            }
        });
        checkCheatMode();
        ...
        API.addOption(new OptionCycled("inventory.layoutstyle", 0)
        {
            ...
            public String getButtonText() {
                return ASOClientUtils.translate("layoutstyle." +
    ...
    private static void linkOptionList() {
        OptionList.setOptionList(new OptionList("aso.options")
        {
            @Override
            ...
            public GuiOptionList getGui(GuiScreen parent, OptionList list, boolean world) {
                return new GuiASOOptionList(parent, list, world);
            }
        });
    }
    ...
    public static OptionList getOptionList() {
        return OptionList.getOptionList("aso.options");
    }
    ...
    public static void loadWorld(String saveName) {
        ...
        bootASO(ClientUtils.getWorld());

        File saveDir = new File(CommonUtils.getMinecraftDir(), "saves/ASO/" + saveName);
        boolean newWorld = !saveDir.exists();
        if (newWorld)
            saveDir.mkdirs();

        world = new ConfigSet(new File(saveDir, "ASO.dat"), new ConfigFile(new File(saveDir, "ASO.cfg")));
        onWorldLoad(newWorld);
    }

    private static void onWorldLoad(boolean newWorld) {
        world.config.setComment("World based configuration of ASO.\nMost of these options can be changed ingame.\nDeleting any element will restore it to it's default value");
        ...
        ASOInfo.load(ClientUtils.getWorld());
    }
    ...
    public static void bootASO(World world) {
        if (configLoaded)
            return;
        ...
        ClassDiscoverer classDiscoverer = new ClassDiscoverer(new IStringMatcher()
        {
            public boolean matches(String test) {
                return test.startsWith("ASO") && test.endsWith("Config.class");
            }
        }, IConfigureASO.class);
        ...
        for (Class<?> clazz : classDiscoverer.classes) {
            try {
                IConfigureASO config = (IConfigureASO) clazz.newInstance();
                config.loadConfig();
                ASOModContainer.plugins.add(config);
    ...
    public static void loadState(int state) {
        ...
        GuiContainer currentContainer = ASOClientUtils.getGuiContainer();
        ...
        for (TaggedInventoryArea area : saveAreas) {
            ...
            for (int slot : area.slots) {
                ASOClientUtils.setSlotContents(slot, null, area.isContainer());
            }
            ...
            for (int i = 0; i < areaTag.tagCount(); i++) {
                ...
                ASOClientUtils.setSlotContents(slot, ItemStack.loadItemStackFromNBT(stacksave), area.isContainer());
            }
        }
    }
    ...
    public static boolean canPerformAction(String name) {
        ...
        String base = ASOActions.base(name);
        if (hasSMPCounterpart)
            return permissableActions.contains(base);

        if (ASOActions.smpRequired(name))
            return false;
    ...
    public static void reloadSaves() {
        File saveDir = new File(CommonUtils.getMinecraftDir(), "saves/ASO/local");
        if (!saveDir.exists())
            return;
