...
import codechicken.aso.ItemList.EverythingItemFilter;
import codechicken.aso.ItemList.ItemsLoadedCallback;
import codechicken.aso.api.ItemFilter.ItemFilterProvider;
import codechicken.aso.config.ArrayDumper;
import codechicken.aso.config.ItemPanelDumper;
import codechicken.aso.config.RegistryDumper;
import codechicken.aso.guihook.GuiContainerManager;
import codechicken.aso.recipe.BrewingRecipeHandler;
import codechicken.aso.recipe.RecipeItemInputHandler;
...
public class ItemInfo
{
    ...
    private static void parseModItems() {
        ...
        for (Item item : (Iterable<Item>) Item.itemRegistry) {
            ...
            if(ident == null) {
                ASOClientConfig.logger.error("Failed to find identifier for: "+item);
                continue;
            }
        ...
        for(Entry<String, ItemStackSet> entry : modSubsets.entrySet()) {
            ...
            if(mc == null)
                ASOClientConfig.logger.error("Missing container for "+entry.getKey());
            else
    ...
    private static void searchItems() {
        ...
        for (Item item : (Iterable<Item>) Item.itemRegistry) {
            ...
            try {
                ...
                ASOClientConfig.logger.error("Error loading brewing ingredients for: "+item, e);
            }
        }
