...
import codechicken.aso.guihook.*;
...
import static codechicken.aso.ASOClientConfig.*;
import static codechicken.aso.ASOClientUtils.*;
...
public class LayoutManager implements IContainerInputHandler, IContainerTooltipHandler, IContainerDrawHandler, IContainerObjectHandler, IKeyStateTracker
{
    ...
    private static void init() {
        ...
        delete = new Button()
        {
            ...
            public boolean onButtonPress(boolean rightclick) {
                ...
                if (held != null) {
                    ...
                    ASOController.toggleDeleteMode();
            ...
            public String getButtonTip() {
                if ((state & 0x3) != 2) {
                    ...
                    if (ASOController.canUseDeleteMode())
        ...
        for (int i = 0; i < 4; i++) {
            ...
            timeButtons[i] = new Button()
            {
                @Override
                public boolean onButtonPress(boolean rightclick) {
                    if (handleDisabledButtonPress(ASOActions.timeZones[zone], rightclick))
                        return true;
                ...
                @Override
                public String getButtonTip() {
                    return getTimeTip(ASOActions.timeZones[zone], state);
                }
    ...
    private static boolean handleDisabledButtonPress(String ident, boolean rightclick) {
        if (!ASOActions.canDisable.contains(ident))
            return false;
    ...
    private static boolean setPropertyDisabled(String ident, boolean disable) {
        if (disable && ASOActions.base(ident).equals("time")) {
            int count = 0;
            for (int i = 0; i < 4; i++) {
                if (disabledActions.contains(ASOActions.timeZones[i]))
                    count++;
            }
            if (count == 3)
                return false;
        ...
        if (hasSMPCounterPart())
            ASOCPH.sendSetPropertyDisabled(ident, disable);
    ...
    public static LayoutStyle getLayoutStyle() {
        return getLayoutStyle(ASOClientConfig.getLayoutStyle());
    }
    ...
    public static void drawIcon(int x, int y, Image image) {
        changeTexture("aso:textures/aso_sprites.png");
