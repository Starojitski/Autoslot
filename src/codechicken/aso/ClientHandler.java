...
public class ClientHandler
{
    ...
    private void updateMagnetMode(World world, EntityPlayerSP player) {
        if (!ASOClientConfig.getMagnetMode()) return;
        ...
        for (Iterator<EntityItem> iterator = SMPmagneticItems.iterator(); iterator.hasNext(); ) {
            ...
            if (!ASOClientUtils.canItemFitInInventory(player, item.getEntityItem())) continue;
    ...
    public static void init() {
        ...
        PacketCustom.assignHandler(ASOCPH.channel, new ASOCPH());
    ...
    public void tickEvent(TickEvent.ClientTickEvent event) {
        ...
        if(mc.theWorld != null) {
            if(loadWorld(mc.theWorld)) {
                ASOClientConfig.setHasSMPCounterPart(false);
                ASOClientConfig.setInternalEnabled(false);

                if (!Minecraft.getMinecraft().isSingleplayer())//wait for server to initiate in singleplayer
                    ASOClientConfig.loadWorld("remote/" + ClientUtils.getServerIP().replace(':', '~'));
            }
            ...
            ASOController.updateUnlimitedItems(mc.thePlayer.inventory);
            if (mc.currentScreen == null)
                ASOController.processCreativeCycling(mc.thePlayer.inventory);
    ...
    public void tickEvent(TickEvent.RenderTickEvent event) {
        if(event.phase == Phase.END && ASOClientConfig.isEnabled())
