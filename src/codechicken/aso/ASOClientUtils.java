...
import codechicken.aso.api.IInfiniteItemHandler;
...
public class ASOClientUtils extends ASOServerUtils
{
    public static LangProxy lang = new LangProxy("aso");
    ...
    public static void setSlotContents(int slot, ItemStack item, boolean containerInv) {
        ASOCPH.sendSetSlot(slot, item, containerInv);
    ...
    public static void cheatItem(ItemStack stack, int button, int mode) {
        ...
        if (mode == -1 && button == 0 && shiftKey() && ASOClientConfig.hasSMPCounterPart()) {
    ...
    public static void giveStack(ItemStack base, int i, boolean infinite) {
        ...
        if (hasSMPCounterPart()) {
            ...
            if (!infinite && !canItemFitInInventory(mc().thePlayer, stack) && (mc().currentScreen instanceof GuiContainer)) {
                ...
                if(given > 0)
                    ASOCPH.sendGiveItem(copyStack(typestack, given), false, false);
            } else
                ASOCPH.sendGiveItem(stack, infinite, true);
        } else {
    ...
    public static int getNextGamemode() {
        ...
        int nmode = mode;
        while (true) {
            nmode = (nmode + 1) % ASOActions.gameModes.length;
            if (nmode == mode || isValidGamemode(ASOActions.gameModes[nmode]))
                break;
        }
        return nmode;
    }
