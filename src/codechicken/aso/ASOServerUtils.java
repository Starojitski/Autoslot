...
public class ASOServerUtils
{
    ...
    public static void toggleRaining(World world, boolean notify) {
        ...
        if (notify)
            ServerUtils.sendChatToAll(new ChatComponentTranslation("aso.chat.rain." + (raining ? "on" : "off")));
    }
    ...
    public static void setHourForward(World world, int hour, boolean notify) {
        ...
        if (notify)
            ServerUtils.sendChatToAll(new ChatComponentTranslation("aso.chat.time", getTime(world) / 24000L, hour));
    }
    ...
    public static void advanceDisabledTimes(World world) {
        ...
        int newhour = hour;
        while (ASOServerConfig.isActionDisabled(dim, ASOActions.timeZones[newhour / 6]))
    ...
    public static void sendNotice(ICommandSender sender, IChatComponent msg, String permission) {
        ...
        if (ASOServerConfig.canPlayerPerformAction("CONSOLE", permission))
            MinecraftServer.getServer().addChatMessage(notice);

        for (EntityPlayer p : ServerUtils.getPlayers())
            if(p == sender)
                p.addChatComponentMessage(msg);
            else if (ASOServerConfig.canPlayerPerformAction(p.getCommandSenderName(), permission))
    ...
    public static void givePlayerItem(EntityPlayerMP player, ItemStack stack, boolean infinite, boolean doGive) {
        if (stack.getItem() == null) {
            player.addChatComponentMessage(setColour(new ChatComponentTranslation("aso.chat.give.noitem"), EnumChatFormatting.WHITE));
            return;
        }
    ...
    public static int getCreativeMode(EntityPlayerMP player) {
        if (ASOServerConfig.forPlayer(player.getCommandSenderName()).isActionEnabled("creative+"))
            return 2;
    ...
    public static void setGamemode(EntityPlayerMP player, int mode) {
        if (mode < 0 || mode >= ASOActions.gameModes.length ||
                ASOActions.nameActionMap.containsKey(ASOActions.gameModes[mode]) &&
                        !ASOServerConfig.canPlayerPerformAction(player.getCommandSenderName(), ASOActions.gameModes[mode]))
            return;

        //creative+
        ASOServerConfig.forPlayer(player.getCommandSenderName()).enableAction("creative+", mode == 2);
        if(mode == 2 && !(player.openContainer instanceof ContainerCreativeInv))//open the container immediately for the client
            ASOSPH.processCreativeInv(player, true);

        //change it on the server
        player.theItemInWorldManager.setGameType(getGameType(mode));

        //tell the client to change it
        new PacketCustom(ASOSPH.channel, 14).writeByte(mode).sendToPlayer(player);
        player.addChatMessage(new ChatComponentTranslation("aso.chat.gamemode." + mode));
    }
    ...
    public static void logOnce(Throwable t, Set<String> stackTraces, String message, String identifier) {
        ...
        if (!stackTraces.contains(stackTrace)) {
            ASOServerConfig.logger.error(message, t);
