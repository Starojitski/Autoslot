...
import codechicken.aso.ItemList.AnyMultiItemFilter;
import codechicken.aso.ItemList.ItemsLoadedCallback;
import codechicken.aso.ItemList.NothingItemFilter;
...
public class SubsetWidget extends Button implements ItemFilterProvider, ItemsLoadedCallback, ISearchProvider
{
    ...
    public static void loadHidden() {
        ...
        try {
            NBTTagList list = ASOClientConfig.world.nbt.getTagList("hiddenItems", 10);
            for(int i = 0; i < list.tagCount(); i++)
                itemList.add(ItemStack.loadItemStackFromNBT(list.getCompoundTagAt(i)));
        }
        catch (Exception e) {
            ASOClientConfig.logger.error("Error loading hiddenItems", e);
            return;
        }
    ...
    private static void saveHidden() {
        ...
        if(list != null) {
            ASOClientConfig.world.nbt.setTag("hiddenItems", list);
            ASOClientConfig.world.saveNBT();
        }
    }

    private static final RestartableTask prepareDirtyHiddenItems = new RestartableTask("ASO Subset Save Thread")
    {
    ...
    private static class UpdateStateTask extends RestartableTask
    {
        ...
        public UpdateStateTask() {
            super("ASO Subset Item Allocation");
        }
